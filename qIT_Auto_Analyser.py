...
def qit_analysis():
    ...
    print("************* PLEASE READ *************\n")
    print(
        "Hello, welcome to the qIT Auto-analyser.\n -This software is used to analyse the results of qIT screens in a 384 well format using the standard output files from a Clariostar plate reader using mars software. \n -See the example data for an example of the file formating, any changes to this standard format (e.g. an extra line or an incomplete plate) will fail.\n -Input files should be in the form of .csv files and should always be named following the pattern - x mins.csv. ANY CHANGES TO THIS FORMAT WILL NOT WORK.\n"
    )
    print(
        "-If you would like your compound structures and descriptor information to appear in the report, an appropriately labelled datawarror file (e.g. DHC_library.dwar) must be present in the current working directory."
    )
    print("To prepare data to analyse, please do the following:")
    print(
        "STEP 1. Convert all input files to csv files by opening in excel > save as > .csv. Name the file following the format - x mins.csv."
    )
    print(
        "STEP 2. Combine these files into a new folder named files_XXX (XXX = a relevant name, e.g. files_IDH1TAP1)"
    )
    print(
        "STEP 3. Place this folder in the inputs folder (if you do not alreay have one, a folder titles inputs should be created in the same folder as this module)"
    )
    print("STEP 4. Ensure the correct GSH data is in the folder marked Glu_data. \n")
    print(
        "-If these instructions have been followed but there are still issues, please contact charliepeterbrown@gmail.com\n"
    )
    construct_name = input(
        "Which dataset would you like to analyse? (Please input the name of you used in the input folder, e.g. if my file was named files_IDH1 I would input IDH1)"
    )
    ...
    df_r2_rate = model(
        glu_lib_q, cmp_info_file, construct_name, glu_data_path, index_list, df_results
    )
    ...
    if glu_lib_q == "No":
        ...
        try:
            df_glu = compound_info_df.merge(
                df_glu_pre, on=["Index Key", "Compounds", "Smile"]
            )
        ...
        if glu_lib_q == "No":
            ...
            df_merged["Rate Glu"] = pd.to_numeric(
                df_merged["Rate Glu"], errors="coerce"
            )
    ...
    if glu_lib_q == "Yes":
        ...
        try:
            df_merged = compound_info_df.merge(
                df_merged_1, on=["Index Key", "Compounds", "Smile"]
            )
        except:
